...
import {
   getPendingQuotationRequestsForSeller,
   getRejectedQuotationRequestsForSeller,
} from "../../actions";
...
export type SellerQuotationRequest = Awaited<
   ReturnType<typeof getRejectedQuotationRequestsForSeller>
>["requests"][number];

export function getRejectedQuotationRequestsInfiniteQueryOptions({
   limit = DEFAULT_LIMIT,
   ...
   Error,
   {
      items: Awaited<ReturnType<typeof getRejectedQuotationRequestsForSeller>>["requests"];
      count: number;
   },
   Page,
   QueryKey,
